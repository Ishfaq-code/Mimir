...
The existing Canvas 2D renderer paints a 20-world-unit grid. Right-click opens copy, paste, cut, and delete. Screenshots still paste with ⌘V / Ctrl+V. The toolbar sits 64 px above the canvas bottom, with zoom/navigation in a separate footer. ResizeObserver maintains the backing store as available width changes.

Select, Pen, Eraser, and Text are the exposed tools. Select supports click/Shift-click selection, marquee selection, multi-element movement, and corner resizing; the pasted screenshot is selectable and transformable as well. Text mode supports tap-to-create and tap-to-edit, Done/Enter to save, Shift+Enter for a new line, and Escape/Cancel to discard edits. Pasting with Ctrl+V / ⌘V while a textbox is open inserts into that box; the editor wraps and grows to show the full text. Text uses the selected ink color and participates in undo/redo and erasing. One finger pans and two fingers pinch to zoom in every canvas tool, including Select. In Text mode a stationary finger tap creates or edits text; dragging still navigates. Apple Pencil contact takes priority over finger navigation, and fingers resting on the canvas during Pencil use are ignored until lifted. Selectable typeset equations share the same input routing. Buttons are 42 px high (40 px minimum width on phones); the selected tool uses pale green. Ink options open in an anchored 320 px popover (300 px on phones), close outside or with Escape, and retain all eight colors and three widths. Undo/redo buttons show disabled state when unavailable.

### Kinematics visualization

Paste a word problem onto the canvas or into a textbox. A compact Visualize chip sits on the top-right corner of that textbox; click it to open the kinematics modal. Pasted text wraps to the viewport, grows to show the full problem, and participates in undo/redo. With no problem on the board, the modal explains the paste-then-chip action; it never generates a replacement question. Premade demo questions live in [docs/DEMO_PROBLEMS.md](docs/DEMO_PROBLEMS.md). The four demo problems use cached frames. Other submitted problems are interpreted through the API, using the examples as guidance. Identical text is reused from cache. The object label follows the problem. Unsupported problems explain what to clarify; provider failures offer retry. The compact modal header shows the full question as a 21 px review heading beside Close, with scroll access for long questions; there is no eyebrow, extra title, or scope paragraph. The dialog retains an accessible name. An unboxed four-column readout names Time, Distance, Speed, and Acceleration, with tabular values and quieter units. The animation uses a tighter horizontal viewport and a separate vertical viewport for falling motion. Inherit the existing paper, green accent, Geist typography, and light/dark themes.

Place on canvas puts the playing visualization on the board as a draggable, resizable overlay, then closes the modal. The modal fades and eases in over about 320 ms. Scene objects arrive once along the motion axis over about 900 ms with a short stagger; playback interpolation is unchanged. Reduced motion and calm leave the overlay and scene in their final state. Results autoplay over eight seconds with smooth interpolation between validated frames. Grouped SVG Backward, Play/Pause, and Forward icon controls sit above the timeline as 44 px targets; Play is the filled accent control and carries its name in the accessible label only. There is no Playing/Paused label. Show elapsed and total simulated time instead of a percentage. Stepping or seeking pauses playback, and Play at the end restarts. Narrow layouts retain all four values and transport controls. Loading, retry, and empty-board guidance remain. The modal retains cancellation, focus restoration, and an inert background.
...
The Typeset math switch remains on the canvas. When strokes convert, the LaTeX overlay bounces in over 420 ms while fading in over 780 ms. Recognition failures preserve ink and expose a retry action. Recognized math and tutor annotations remain aligned with world coordinates. Provider configuration is required for voice and stroke conversion. Screenshot OCR is local and needs no provider credentials.
...
The header includes a compact **I write / AI writes** segmented control before Learning tools. Native radio inputs support keyboard arrows and a visible focus outline; the selected segment uses accent-soft and accent. Both options remain visible before and during a conversation. Hover descriptions explain who writes without adding permanent helper copy. Narrow layouts remove the segment icons and retain the labels. The current choice is saved with learning preferences; it does not introduce a sidebar or disable the pen.

In I write, normally guide the student and write only on an explicit request. In AI writes, handle handwriting one checked step at a time while the student answers. Writing uses the latest complete related ink line to estimate symbol height and pen thickness. Each new full expression is centered directly below that line; preserve the established column when the model selects just an answer inside a tutor blank. The SVG and tutor snapshot share those measurements. Keep the hand-drawn glyph style and accent color; do not claim to reproduce the student's personal handwriting. A checked spoken answer can fill a tutor blank in place, retaining its scale and horizontal center as the line width changes. Retain parentheses and unaffected terms through each step. A written answer remains untouched, with the checked full expression added below. Wide lines retain their size on the infinite canvas, and repeated tool calls reuse the same annotation. Never replace a blank containing student ink.

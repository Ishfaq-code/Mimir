...
Tutor writing is opt-in: normally guide the student to write the step themselves. Only an explicit request to the AI can create or fill a step. Requested writing uses the latest related ink line to estimate symbol height, pen thickness and left alignment. The SVG and tutor snapshot share those measurements. Keep the hand-drawn glyph style and accent color; do not claim to reproduce the student's personal handwriting. On request, a checked answer can fill a tutor blank in place without changing its scale or anchor. Wide lines retain their size on the infinite canvas, and repeated tool calls reuse the same annotation. Never replace a blank containing student ink.

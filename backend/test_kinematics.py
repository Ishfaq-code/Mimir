...
class KinematicsTests(unittest.TestCase):
    def test_pasted_demo_questions_without_provider(self):
        with patch("main.is_openrouter_visualization_configured", return_value=False), patch("main.extract_physics", new_callable=AsyncMock) as extract:
            client = TestClient(app)
            for kind in ("speed_up", "braking", "constant_speed", "free_fall"):
                demo = generate_kinematics_question(kind, random.Random(42))
                response = client.post("/visualize", json={"topic": "kinematics", "problem": "\n".join(demo.problem.split())})
                self.assertEqual(response.status_code, 200)
                self.assertEqual(response.json(), demo.visualization.model_dump())
            response = client.post("/visualize", json={"topic": "kinematics", "problem": "A different problem"})

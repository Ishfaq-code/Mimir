...
- Docker hot-reloads all three services: the backend mounts `./backend` with Uvicorn reload, the frontend mounts `./frontend` running `next dev` (Turbopack) with a named volume for `node_modules`, and the LiveKit agent mounts `./agent` running `python main.py dev`. Backend and agent load their own ignored `.env` files; `LIVEKIT_AGENT_NAME` must match and be unique per developer sharing a LiveKit project. Do not run a second manual worker with that name. Recreate backend/agent after environment changes. After frontend dependency changes, run `docker compose exec frontend npm install` (or `docker compose down -v && docker compose up --build`); after Python dependency changes, rebuild the relevant image. The frontend Dockerfile still produces the standalone production image as its default target; Compose builds its `dev` stage.
